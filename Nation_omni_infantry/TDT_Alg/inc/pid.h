...
typedef struct _PidParam
{
	...
	float ki=0;				   ///<积分系数
	float kd=0;				   ///<微分系数
	float integralErrorMax=0;	   ///<积分限幅
	float resultMax=0;		   ///<最终结果限幅
	u8 positiveFBFlag=0;		   ///<正反馈标志位，为1时输出会*(-1)1
	float integralThreshold=0;   ///<积分分离阈值
	float kpAmpForIntegralSep=0; ///<误差大于积分分离阈值时的放大系数
	u8 integralMethod=0;		   ///<积分方法【正常=0，积分分离=1，变积分=2, 变积分2 =3】
	u8 differentialMethod=0;	   ///< 微分算法【正常=0，微分先行=1，加低通滤波】
	///以下参数是更加高级也更加少用的参数
	///@warning 部分参数及算法未测试
	struct
	{
