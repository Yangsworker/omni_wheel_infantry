...
union GraphicDataUnion batteryGraphic[6]=
{
  //电池条
	{0,7,2,  1,0,5,2,  0,0,  8,1770,870,10,1770,830},
	{0,7,3,  1,0,5,2,  0,0,  8,1785,870,10,1785,830},
	{0,7,4,  1,0,5,2,  0,0,  8,1800,870,10,1800,830},
	{0,7,5,  1,0,5,2,  0,0,  8,1815,870,10,1815,830},
};
...
#define calibrationX (MIDDLE_X+7)//infantry1: -3;  infantry2: +7;
//起始x
#define L2for15_X (calibrationX-80)
#define L3for15_X (calibrationX-40)
#define L4for15_X (calibrationX-20)
#define L45for15_X (calibrationX-15)
#define L5for15_X (calibrationX-13)
#define L55for15_X (calibrationX-14)
#define L6for15_X (calibrationX-11) //15
#define LL1 (calibrationX-9)
#define LL (calibrationX-7)

union GraphicDataUnion Auxiliary[] = { //射表
	{0, 8, 0, 1, 0, 2, 2, 0, 0, 1, calibrationX, 500, 0, calibrationX, 200},//竖线
	
	{0,8,1, 1,0,2,2, 0, 0, 2, L2for15_X, 500-50*1, 0, calibrationX*2-L2for15_X, 500-50*1}, 
	{0,8,3, 1,0,2,3, 0, 0, 2, L3for15_X, 500-50-15*1, 0, calibrationX*2-L3for15_X, 500-50-15*1}, 
  {0,8,5, 1,0,2,6, 0, 0, 2, L4for15_X, 500-50-15*2, 0, calibrationX*2-L4for15_X, 500-50-15*2}, 
	
	{0,8,6, 1,0,2,6, 0, 0, 2, L45for15_X, 500-50-15*3, 0, calibrationX*2-L45for15_X, 500-50-15*3}, 
	{0,8,2, 1,0,2,2, 0, 0, 2, L55for15_X, 500-50-15*4, 0, calibrationX*2-L55for15_X, 500-50-15*4}, 	
};  

union GraphicDataUnion Auxiliary1[] = { //射表

	{0,8,10, 1,0,2,5, 0, 0, 2, L5for15_X, 500-50-15*5, 0, calibrationX*2-L5for15_X, 500-50-15*5}, 
	{0,8,11, 1,0,2,6, 0, 0, 2, L6for15_X, 500-50-15*6, 0, calibrationX*2-L6for15_X, 500-50-15*6}, 
	{0,8,12, 1,0,2,6, 0, 0, 2, LL1, 500-50-15*7, 0, calibrationX*2-LL1, 500-50-15*7}, 
	{0,8,13, 1,0,2,6, 0, 0, 2, LL, 500-50-15*8, 0, calibrationX*2-LL, 500-50-15*8},
	//电池框
	{0,7,0,  1,1,5,8,  0,0,  2,1745,860,10,1760,840},
	{0,7,1,  1,1,5,8,  0,0,  2,1760,875,10,1825,825},
...
//}
void clientUpdataData()  //更新函数1
{ 
//********************************主控/视觉（第一个灯）**************************
	if(gimbalOffline) 
	{ 
		dynamicGraphic[0].GraphicData.color = COLOR_MODE1; //橙 （云台离线/主控离线）
	} 
	else if(canStateStruct.visionOffline)
	{ 
		dynamicGraphic[0].GraphicData.color = COLOR_MODE2; //青 （视觉离线）
	} 
	else 
	{ 
		dynamicGraphic[0].GraphicData.color = COLOR_DISABLE; //白 （正常）
	} 
//*********************************摩擦轮（第二个灯）******************************	 
    switch(canStateStruct.frictionSpeed_State)
		{
			case 0:
				dynamicGraphic[1].GraphicData.color = 7;//黑 （有摩擦轮电机离线）
				break;
			case 1:
				dynamicGraphic[1].GraphicData.color = 1; //黄  （未打开摩擦轮）
				break;
			case 2:
				dynamicGraphic[1].GraphicData.color = 8;//白  （已开启摩擦轮）
				break;
		}
...
//	} 
//********************************************弹频（第三个灯）*************************************** 
	if(canStateStruct.fireFrequencyMode) 
	{ 
...
//********************************************功率百分比数字********************************** 
	...
	{ 
		dynamicGraphic[3].intData.color = JGMT_COLOR_ORANGE; //橙色
	...
	{ 
		dynamicGraphic[3].intData.color = JGMT_COLOR_CYAN; //青色
	} 
...
void clientUpdataData3() //底盘/视觉锁
{ 
	for(int i = 0; i < 4; i++) 
	{ 
		if(++can1Feedback.chassisOfflineCheck[i]>1) 
		{ 
			can1Feedback.chassisOffline[i]=1;   //底盘离线（橙色为可能离线，青色为已经离线，黑色为电机温度高于60度）
		...
		{ 
			if(can1Feedback.chassisTemperature[i] > 60) 
...
//	memcpy(&dynamicGraphic5[6], &blockErrorGraphic, sizeof(blockErrorGraphic)); 
	...
	u8 batteryLevel = LIMIT((uint8_t)((jgmt_mesg.powerHeatData.chassisVolt - 19000)/1000),1,5);
	
	batteryGraphic[0].GraphicData.color = (batteryLevel>3)?2:((batteryLevel>2)?3:4);
	batteryGraphic[0].GraphicData.width = (jgmt_mesg.powerHeatData.chassisVolt>24000)?4:0;
	
	batteryGraphic[1].GraphicData.color = batteryGraphic[0].GraphicData.color;
	batteryGraphic[1].GraphicData.width = (jgmt_mesg.powerHeatData.chassisVolt>23000)?4:0;
	
	batteryGraphic[2].GraphicData.color = batteryGraphic[0].GraphicData.color;
	batteryGraphic[2].GraphicData.width = (jgmt_mesg.powerHeatData.chassisVolt>22000)?4:0;
	
	batteryGraphic[3].GraphicData.color = batteryGraphic[0].GraphicData.color;
	batteryGraphic[3].GraphicData.width = (jgmt_mesg.powerHeatData.chassisVolt>21000)?4:0;

  memcpy(dynamicGraphic5, &batteryGraphic, sizeof(batteryGraphic));
//	if(canStateStruct.ammoCoverOpen) 
//	{ 
...
void clientUpdataData6()
{
	...
	if(canStateStruct1.OffsetModifyFlag)
	{
		...
		if(LastOffsetModifyMode!=canStateStruct1.OffsetModifyMode)   //检测到变化重新刷新
		{
			for(u8 i=0;i<4;i++)
			{
...
{ 
	/*开始比赛自动刷新，不开比赛手动刷新*/
	if(jgmt_mesg.gameStatus.gameProgress == 3)
	   UI_InitFlag = 1;
	else 
		 UI_InitFlag = canStateStruct.UI_Init;
...
//		AuxiliaryUpdate();
	...
	if(schedule1ReDraw == 208) //射表2/电池框
	{ 
//		AuxiliaryUpdate();
