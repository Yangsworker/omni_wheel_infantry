...
u8 Com_taskCounter = 0;
int recvComCounter=0;
int generalCounter=0;
//int generalCounterMod=0;
void Loop_1000Hz(void)	//1ms执行一次
{
	...
	{
		MidTimer = 0;
		Send_Judgement_Robot_Data();  //0x111
//		Send_Judgement_Robot_Data2();
		Send_Judgement_Game_Data();   //0x112
	}
	...
	Com_taskCounter++;
	recvComCounter++;
	if(jgmtDrawCounter > 34)//34
	{
		jgmtDrawCounter = 0;
//		if(Com_taskCounter!=0 || !canStateStruct.commandguardFlag)
//		{
			draw_schedule1();
//		}
	}

	if(Com_taskCounter > 200)//67
	{
		Com_taskCounter = 0;
		if(jgmtDrawCounter!=0)
		{
  		//send_schedule1();
		}
	}
		if(recvComCounter>50)//50
	{
		recvComCounter=0;
		if(jgmtDrawCounter!=0 && Com_taskCounter!=0)
		{
			//recv_COM_schedule();
		}
	}	
...
#include "can.h"
void Loop_500Hz(void)	 //2ms执行一次（由主控发送） //修改标志
{
