...
///J-scope数据测试，互补滤波算法解算出来的角度
eulerAngle angleFromLib;
///J-scope数据测试，大疆开源库AHRS算法解算出来的角度
...
bool ImuCalc::gyroAutoCalibration()
{
	static uint16_t runningTimes;
	 ...
	 gyro.dps.data[x] = gyro.origin.data[x]* gyroDpsFector;
	 gyro.dps.data[y] = gyro.origin.data[y]* gyroDpsFector;
	 gyro.dps.data[z] = gyro.origin.data[z]* gyroDpsFector;
	...
	if (runningTimes == 0)
	{
		...
		gyro.offset_max.data[x] = -FLT_MAX;
		gyro.offset_max.data[y] = -FLT_MAX;
		gyro.offset_max.data[z] = -FLT_MAX;
		gyro.offset_min.data[x] = FLT_MAX;
		gyro.offset_min.data[y] = FLT_MAX;
		gyro.offset_min.data[z] = FLT_MAX;
	}

	if (gyro.dps.data[x] > gyro.offset_max.data[x])
		gyro.offset_max.data[x] = gyro.dps.data[x];
	if (gyro.dps.data[y] > gyro.offset_max.data[y])
		gyro.offset_max.data[y] = gyro.dps.data[y];
	if (gyro.dps.data[z] > gyro.offset_max.data[z])
		gyro.offset_max.data[z] = gyro.dps.data[z];

	if (gyro.dps.data[x] < gyro.offset_min.data[x])
		gyro.offset_min.data[x] = gyro.dps.data[x];
	if (gyro.dps.data[y] < gyro.offset_min.data[y])
		gyro.offset_min.data[y] = gyro.dps.data[y];
	if (gyro.dps.data[z] < gyro.offset_min.data[z])
		gyro.offset_min.data[z] = gyro.dps.data[z];
	...
	if (gyro.offset_max.data[x] - gyro.offset_min.data[x] > 1.5 ||
		gyro.offset_max.data[y] - gyro.offset_min.data[y] > 1.0 ||
		gyro.offset_max.data[z] - gyro.offset_min.data[z] > 1.0	){
		runningTimes = 0;
	}
...
//	if(ABS(gyro.calibration.data[z])<3) gyro.calibration.data[z]*=0.9*(1-(ABS(gyro.calibration.data[z]))/3);
