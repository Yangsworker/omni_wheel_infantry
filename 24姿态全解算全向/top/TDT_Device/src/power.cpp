...
//    }
//	//******************功率异常，裁判系统正常->软硬件共同限制******************//
//		else if(!JgmtPack.jgmtOfflineFlag)
//	{
...
//	if(!ctrlCnt++ && JgmtPack.chassisPowerBuffer <= 0)//缓冲能量使用到限制以下，增大限制强度
//	{
//		overFlowKp -= 0.03f;
...
//	if(!JgmtPack.jgmtOfflineFlag && txCnt)  //裁判系统离线不处理,不发送,由功率模块处理
//	{ 
//		chassisPower  =  JgmtPack.;
//		chassisPowerBuffer = LIMIT(JgmtPack.powerHeatData.chassisPowerBuffer, 0, 300);   //底盘缓冲能量,限幅，防止充不进去电?
//		max_chassis_power = LIMIT(JgmtPack.gameRobotStatus.chassisPowerLimit, 50, 100);  //最大功率，限幅，防止裁判系统乱发数导致通道超载
//		  
