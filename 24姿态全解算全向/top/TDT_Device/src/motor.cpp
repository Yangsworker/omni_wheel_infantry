...
///大疆电机对象列表
MotorList motorList[2][12];
///电机类大疆电机标志位初始化
u8 Motor::DJI_MotorFlag[2][12] = {0};
///大疆电机CAN发送缓冲区
...
float Motor::ctrlCurrent(float current, u8 sendFlag)
{
	...
	if (sendFlag && enableFlag.canSendMsg)
	{
		...
		if (this->otherInfo.isDjiMotorFlag == 0)
		{
			if(deforceFlag==1) 
			{
         otherMotorFunction(0, this);
			   return 0;
			}
			//参数检查
			else if(this->otherMotorFunction != 0)
			{
...
u8 Motor::isDJIMotor(CAN_TypeDef *_CANx, uint32_t _Std_ID)
{
	...
	{
		if (_Std_ID < 0x201 || _Std_ID > 0x20C)
		{
			return 0;
		}
		else
...
//	int can_i = 1;
	for (int can_i = 0; can_i < 2; can_i++)
	{
		...
		for (int idIndex = 0; idIndex < 3; idIndex++)
		{
			...
			canTx(&canBuff[can_i][idIndex * 4], can_i == 0 ? CAN1 : CAN2, idIndex == 0 ? 0x200 : (idIndex == 1 ? 0x1ff : 0x2ff));
		}
	}
}
