 ...
 * @ingroup TDT_DEVICE
 * @brief 只需通过 USE_JUDGEMENT 宏定义即可开启读取，协议内容具体参考裁判系统协议文档
 */
float shootingSpeed[3] ;
...
#define USE_DMA_TRANSFER_DATA 1 //使用DMA进行内存拷贝
...
switch (cmdid)
	...
	case POWER_HEAT_DATA:
		batteryLevel = LIMIT((uint8_t)((powerHeatData.chassisVolt-19000)/1000),1,5);	
	default:
		break;
	}
}
